...
function plot_genetic_op_performances(experiment_list, directory_list, selected_exp_no, run_no)
...
% check if file exists in the experiment directory
filename = 'adaptation_data.txt'; 
run_dir = [directory_list{1} sep 'run_' num2str(run_no)]
found = search_file(run_dir,filename);
directory_name = directory_list{selected_exp_no};    

disp([directory_list{1}  sep 'run_' num2str(run_no) sep  filename])
disp('')

if (found)
    file = [directory_name sep 'run_' num2str(run_no) sep filename];
    % load the data (rows = generations, columns = runs)
    D = dlmread(file,' ',4,0);   % skip header....
    sD = size(D)
else
...
mut_rate = D(:,5); 
repr_tot = D(:,6);
reproduction_perf = D(:,7:9);
repr_av_delta = D(:,10:12);
cross_tot = D(:,13);
crossover_perf = D(:,14:16);
cross_av_delta = D(:,17:19);
tot_smut = D(:,20);
s_mutation_perf = D(:,21:23);
smut_av_delta = D(:,24:26);
tot_pmut = D(:,27);
p_mutation_perf = D(:,28:30);
pmut_av_delta = D(:,31:33);
...
line1_title = ['Run no. ' num2str(run_no)];
line2_title = 'Reproduction';
title({line1_title; line2_title},'FontSize',size_title);
xlabel('Generation', 'FontSize',size_label);
ylabel('%', 'FontSize',size_label);
grid on
...
legend(s_legend, 'interpreter', 'none', 'FontSize', size_label)
line1_title = ['Run no. ' num2str(run_no)];
line2_title = 'Genetic operators rates';
title({line1_title; line2_title},'FontSize',size_title);
xlabel('Generation', 'FontSize',size_label);
