...
function plot_depth_dynamics(experiment_list, directory_list)

%disp('Function plot_depth_dynamics called...')
...
for j=1:n_experiments
    ...
    if (found)
        ...
        m = ['depth_ave.txt not found in ' directory_list{j}];
        disp(m)
    end
...
title('Average depth of archive individuals','FontSize',size_title);
xlabel('Generation', 'FontSize',size_label);
ylabel('Depth', 'FontSize',size_label);
